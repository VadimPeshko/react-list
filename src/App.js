...
function App() {
  ...
  const onEditTask = (listId, taskObj) => {
    const newTaskText = window.prompt('Введите название', taskObj.text)
  ...
  useEffect(() => {
    const urlId = location.pathname.split('lists/')[1];
    if (lists) {
      const list = lists.find(list => list.id === Number(urlId))
      setActiveItem(list)
    }
